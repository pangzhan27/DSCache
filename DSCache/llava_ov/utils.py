...
class StreamingArgs:
    def __init__(self, device):
        self.input_ids = torch.empty((1, 0), dtype=torch.int64, device=device)  # (batch_size, n_head_kv, 0, dim_head)
        self.video_grid_thw = None

    def copy(self):
        new = StreamingArgs(self.input_ids.device)
        new.input_ids = self.input_ids.clone()
        new.video_grid_thw = ( None if self.video_grid_thw is None else self.video_grid_thw.clone())
        return new
